...
def dashboard():
    ...
    SELECT 
        ...
        FROM subscriptions 
    JOIN service  
        ON subscriptions .service _id = service .service _id
    LEFT JOIN billing 
        ON subscriptions .uuid_id = billing.subscription_id
    WHERE subscriptions .user1_id = %s
""", (session["user_id"],))
    subscriptions _list=cur.fetchall()
    
    print(subscriptions _list)
    ...
    for sub in subscriptions _list:
    ...
    conn.close()
    return render_template("dashboard.html",subscriptions =subscriptions _list,active=active,paused=paused,due=due,total=total,)
...
def addservice ():
    if(request.method=="POST"):
        service _name=request.form["service _name"]
        service _provider=request.form["provider"]
        service _provider_number=request.form["number"]
        service _provider_address=request.form["address"]
        price_per_day=request.form["price"]
        daily_quantity=request.form["quantity"]
        start_date=request.form["start"]
        end_date=request.form["end"]
        #empty
        if not service _name or not service _provider  or not service _provider_number or not service _provider_address or not price_per_day or not daily_quantity or not start_date or not end_date:
            flash("All fields are required","error")
            return redirect(url_for("addservice "))
        #regex
        service _name_pattern = r"^[A-Za-z]+( [A-Za-z]+)*$"
        ...
        cur.execute("select service _id,service _name,price_per_day from service  where service _name=%s",(service _name,))
        service =cur.fetchone()
        
        
        #insert service 
        if not service :
            cur.execute("insert into service  (service _name,price_per_day) values (%s,%s)",(service _name,price_per_day))
            cur.execute("select service _id,service _name,price_per_day from service  where service _name=%s",(service _name,))
            service =cur.fetchone()

       #existing service  provider 
        cur.execute("select service _provider_id,service _provider_name,service _provider_number,service _provider_address,service _id from service _provider where service _provider_number=%s",(service _provider_number,))
        service _provider_details=cur.fetchone()
        
        
        #insert service  provider
        if not service _provider_details:
            cur.execute("insert into service _provider(service _provider_name,service _provider_number,service _provider_address,service _id)values (%s,%s,%s,%s)",(service _provider,service _provider_number,service _provider_address,service ["service _id"]))
            cur.execute("select service _provider_id,service _provider_name,service _provider_number,service _provider_address from service _provider where service _provider_number=%s",(service _provider_number,))
            service _provider_details=cur.fetchone()
           
        
        #existing susbcription

        cur.execute("select * from subscriptions  where user1_id=%s and service _id=%s and service _provider_id=%s and start_date=%s and end_date=%s",(user_id,service ["service _id"],service _provider_details["service _provider_id"],start_date,end_date))
        ...
        cur.execute("insert into subscriptions (user1_id,service _id,service _provider_id,start_date,end_date,daily_quantity) values(%s,%s,%s,%s,%s,%s)",(user_id,service ["service _id"],service _provider_details["service _provider_id"],start_date,end_date,daily_quantity))
        flash("subscription added successfully","success")
        print("Insert executed")
        
        cur.execute("""select subscriptions .uuid_id,
                    service .service _id,subscriptions .start_date,subscriptions .end_date,subscriptions .status,subscriptions .user1_id,service .service _name FROM subscriptions  JOIN  service  on subscriptions .service _id=service .service _id where subscriptions .user1_id=%s""",(session["user_id"],))
...
def delete_service (id):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("select service _id,service _provider_id from subscriptions  where uuid_id= UUID_TO_BIN(%s)",(id,))
    service _details=cur.fetchone()
    print(service _details)
    ...
    conn.commit()
    conn.close()
    print("service  id that we have to delete ",service _details)
    
    conn = get_connection()
    cur = conn.cursor()

    cur.execute("select * from subscriptions  where service _id=%s",(service _details["service _id"],))
    exist_subs_dlt_service =cur.fetchone()
    
    if not exist_subs_dlt_service :

        cur.execute("delete from service _provider where service _id=%s",(service _details["service _id"],))
        cur.execute("delete from service  where service _id=%s",(service _details["service _id"],))
...
def generate_bill(id):
    ...
    SELECT s.start_date, s.end_date, s.daily_quantity, s.user1_id,
           srv.price_per_day, srv.service _name,
           sp.service _provider_name,
           sp.service _provider_number
    FROM subscriptions  s
    JOIN service  srv ON s.service _id = srv.service _id
    JOIN service _provider sp ON s.service _provider_id = sp.service _provider_id
...
""", (id,))
    ...
    return render_template(
    "bill.html",
    service _name=sub["service _name"],
    provider_name=sub["service _provider_name"],   # added
    provider_number=sub["service _provider_number"], #
...
def edit_service (id):
    ...
    cur.execute("""
        SELECT s.start_date, s.end_date, s.daily_quantity,
               sp.service _provider_name, sp.service _provider_number, sp.service _provider_address,
               srv.service _name, srv.price_per_day
        FROM subscriptions  s
        JOIN service  srv ON s.service _id = srv.service _id
        JOIN service _provider sp ON s.service _provider_id = sp.service _provider_id
        WHERE s.uuid_id = UUID_TO_BIN(%s)
    ...
    if request.method == "POST":
        ...
        if not service _name or not service _provider  or not service _provider_number or not service _provider_address or not price_per_day or not daily_quantity or not start_date or not end_date:
            ...
            return redirect(url_for("addservice "))
        #regex
        service _name_pattern = r"^[A-Za-z ]{3,}$"
        ...
        cur.execute("""
            UPDATE service  
            SET service _name=%s, price_per_day=%s
            WHERE service _id = (
                SELECT service _id FROM subscriptions  WHERE uuid_id=UUID_TO_BIN(%s)
            )
        """, (service _name, price_per_day, id))

        
        cur.execute("""
            UPDATE service _provider 
            SET service _provider_name=%s,
                service _provider_number=%s,
                service _provider_address=%s
            WHERE service _provider_id = (
                SELECT service _provider_id FROM subscriptions  WHERE uuid_id=UUID_TO_BIN(%s)
            )
        """, (service _provider, service _provider_number, service _provider_address, id))
